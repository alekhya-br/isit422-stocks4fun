...
/*  * Model  *  portfolio must match the name of your collection in mlab mongo db */
...
router.get('/', function (req, res) {
    res.send('no portfolio here!');
});

router.get('/api/portfolio_data', function (req, res) {
    mongoose.connect(conString, () => {    // like SQL, we first connect, then issue cmds
        portfolio.find({}, function (err, docs) {  // find({} gets all, docs is the name of the return json
            var onePortfolioArray = [];  // our new array we will return instead of TODOS array
            if (!err) {
                docs.forEach(function (onePortfolio, index) {  // fill up our array from mongodb's json
                    onePortfolioArray[index] = onePortfolio;    // onePortfolio is temp variable, like item in a C# foreach
                });
                res.send(onePortfolioArray);  // return our array
...
router.post('/api/portfolio_data', function (req, res) {
    try {
        mongoose.connect(conString, () => {   // again, first we connect
            PortfolioObject = req.body;  // get the posted data from angular
            PortfolioObject._id = new ObjectID(); // set our  empty _id with new, unique _id key from mongo
            var aPortfolio = new portfolio(PortfolioObject); // not sure exactly how this works
            aPortfolio.save();   // but we are creating a user object that matches what mongoose wants
...
router.delete('/api/portfolio_data/:id', function (req, res) {
    try {
        mongoose.connect(conString, () => {   // connect
            ...
            portfolio.findByIdAndRemove(mongoid, (err, portfolio) => {   // nice mongoose method
...
router.put('/api/portfolio_data/:id', function (req, res) {  // modify existing Portfolio using an http put
    try {
        mongoose.connect(conString, () => {   // connect
            var mongoid = mongoose.Types.ObjectId(req.params.id);   // convert the angular _id to the object type for mongo
            var portfolioWithNoID = {}; // cannot pass in an object with an _id, as you are not allowed
            portfolioWithNoID.uid = req.body.uid;
            portfolioWithNoID.lastname = req.body.lastname;
            portfolioWithNoID.name = req.body.name;    // to update the _id, so make a new copy object but leave out the _id
            portfolioWithNoID.stocks = req.body.stocks;
            portfolio.findByIdAndUpdate(mongoid, portfolioWithNoID, (err, portfolio) => {
                if (err) {
